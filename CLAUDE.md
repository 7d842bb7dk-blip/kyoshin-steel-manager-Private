...
- **常駐**：ログオン時にスタートアップの `kyoshin-steel.vbs` → `server-daemon.bat` が隠し起動。
  ...
  さらにタスクスケジューラの **「鋼材在庫システム 見張り」**（ログオン時＋5分ごと・管理者権限不要）が
  同じ vbs を起動し、見張りループごと止まっていても最長5分で復帰する（動いていれば何もしない）。
  **Claude などのツールのシェルから server-daemon.bat を直接起動しないこと**（そのセッションが終わると
  一緒に止まる。2026-09-24 夜にこれで停止した）。手動で起動するときは
  `Start-ScheduledTask -TaskName "鋼材在庫システム 見張り"`。
