...
- **HTTPS**：`https://192.168.1.107:3443/` も同じアプリを配信（スマホの「かざすだけQRスキャン」用。
  カメラAPIがHTTPS必須のため）。自己署名証明書は `server/data/tls/`（Git対象外）、
  IPが変わったら `tools/TLS証明書を作る.bat` で作り直す。QRラベルのリンク先は http のまま。
